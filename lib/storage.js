...
async function createShoppingList(listData) {
  ...
  const newList = {
    ...
    maxBudget: listData.maxBudget || null,
...
async function addProductToShoppingList(listId, productId) {
  ...
  const list = data.shoppingLists[listIndex];
  if (!list.items) list.items = [];

  // Avoid duplicates
  const exists = list.items.some(item => item.productId === productId);
  if (!exists) {
    list.items.push({
      productId: productId,
...
async function clearShoppingList(listId) {
  ...
  data.shoppingLists[listIndex].items = [];
  data.shoppingLists[listIndex].updatedAt = new Date().toISOString();
  await saveAllData(data);
  return true;
}

async function toggleShoppingListItem(listId, productId, checked) {
  const data = await getAllData();
  if (!data.shoppingLists) return false;
...
async function getProductShoppingLists(productId) {
  ...
  return data.shoppingLists.filter(l => {
    const items = l.items || [];
